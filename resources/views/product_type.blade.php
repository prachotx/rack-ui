...
@section('content')
    <div class="shadow rounded-box mx-auto">
            ...
            <div class="flex-none gap-2">
                ...
                <div class="dropdown dropdown-end">
                    @if (Auth::user()->role == 'admin')
                        <a class="btn btn-success text-base-100" href="/add_product_type"><i class="fa-solid fa-plus"></i>
                            Add</a>
                    @endif
                </div>
            </div>
        ...
        <div class="p-[10px] border-base-400 border-2 border-t-0 rounded-b-box">
                ...
                <tbody class="text-[16px]">
                    @foreach ($product_types as $item)
                        <tr>
                            ...
                            <td>
                                @if (Auth::user()->role == 'admin')
                                    <a class="btn btn-warning text-base-100" href="/edit_product_type/{{ $item->id }}"><i
                                            class="fa-solid fa-edit"></i></a>
                                    <a class="btn btn-error text-base-100 delete_product_type" data-item="{{ $item }}"
            ...
            {{ $product_types->links('vendor.pagination.tailwind') }}
        </div>
    </div>

    <!-- Modal -->
    <div id="DeleteProductTypeModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white rounded-lg shadow-lg w-1/3">
            <form id="form-product_type" action="" method="post">
                @csrf
                <div class="flex justify-between items-center p-4 border-b">
                    <h5 class="text-lg font-semibold">Delete ProductType</h5>
                </div>
                <div class="p-4">
    ...
    <script>
        //todo:slug generate
        $(document).on('click', '.delete_product_type', function(e) {
            let item = $(this).data('item');
            $('.name').text(item['name']);
            document.getElementById("form-product_type").action = "/delete_product_type/" + item['id'];
            $('#DeleteProductTypeModal').removeClass('hidden');
        });

        $('#closeModalBtn').on('click', function() {
            $('#DeleteProductTypeModal').addClass('hidden');
        });
    </script>
@endsection
